...
pub const RULES: &[RtkRule] = &[
    ...
    RtkRule {
        pattern: r"^gsutil\s+(ls|cp|mv|rsync|cat|stat|du|mb|rb|rm)\b",
        rtk_cmd: "rtk gsutil",
        pipeline_final_safe: false,
        rewrite_prefixes: &["gsutil"],
        category: "Infra",
        savings_pct: 70.0,
        subcmd_savings: &[
            ("ls", 65.0),
    ...
    RtkRule {
        pattern: r"^uv\s+(run|sync|lock|add|remove|tree|pip|init|venv)\b",
        rtk_cmd: "rtk uv",
        pipeline_final_safe: false,
        rewrite_prefixes: &["uv"],
        category: "Python",
        savings_pct: 75.0,
        subcmd_savings: &[("run", 85.0), ("sync", 70.0), ("pip", 65.0), ("lock", 70.0)],
        subcmd_status: &[],
    },
    RtkRule {
