...
enum Commands {
    ...
    /// Google Cloud CLI with compact output
    Gcloud {
        /// gcloud arguments (group + subcommand + flags)
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },

    /// Google Cloud Storage with compact output
    Gsutil {
        /// gsutil arguments (subcommand + flags)
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    ...
    Grep {
        ...
        // already behaves). --max keeps its long form. Optional so the Codex
        // profile can detect an explicit cap and apply its own grep limits.
        #[arg(long)]
        max: Option<usize>,
    ...
    /// uv package manager with compact output
    Uv {
        /// uv arguments (subcommand + flags)
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
...
fn run_cli() -> Result<i32> {
    ...
    let code = match cli.command {
        ...
        Commands::Gcloud { args } => gcloud_cmd::run(&args, cli.verbose)?,
